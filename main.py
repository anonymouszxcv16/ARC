...
def train_online(RL_agent, env, eval_env, args):
    ...
    # Train loop.
    for t in range(int(args.max_timesteps + 1)):
        maybe_evaluate_and_print(RL_agent, eval_env, evals, times, alphas, stds, t, start_time, args)
        ...
        # Do a step.
        next_state, reward, ep_finished, trunc, _ = env.step(action)
...
# Logs.
def maybe_evaluate_and_print(RL_agent, eval_env, evals, times, alphas, stds, t, start_time, args):
    if t % args.eval_freq == 0:
        # Rewards
        total_reward = np.zeros(args.eval_eps)

        for ep in range(args.eval_eps):
            state, done, trunc = eval_env.reset()[0], False, False
            step = 0
