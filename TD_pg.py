...
class Agent(object):
    ...
    def init(self):
        ...
        # Experience Replay
        self.experience_replay = buffer.LAP(self.state_dim, self.action_dim, self.device, self.args, self.args.buffer_size,
    ...
    def estimate_loss(self, replay):
        ...
        if replay.size > 0:
            state, action, _, next_state, reward, _, not_done = replay.sample()

            with torch.no_grad():
                next_action, _ = self.actor_target(next_state, deterministic=True)
                Q_next = self.critic_target(next_state, next_action,).min(1, keepdim=True)[0]
    ...
    def train(self):
        ...
        state, action, log_policy_old, next_state, reward, mc_score, not_done = self.experience_replay.sample()

        # Update Critic.
        with torch.no_grad():
            if not "REINFORCE" in self.args.policy:
                # State next.
                next_action, next_action_log_prob = self.actor_target(next_state, deterministic=True)
        ...
        if self.training_steps % self.hp.policy_freq == 0 or "REINFORCE" in self.args.policy:
            actor, log_policy = self.actor(state)
            ...
            if "REINFORCE" in self.args.policy:
                ...
                self.experience_replay.log_policy[self.experience_replay.ind] = log_policy
